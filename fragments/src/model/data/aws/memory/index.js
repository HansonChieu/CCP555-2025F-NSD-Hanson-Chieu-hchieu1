...
async function deleteFragment(ownerId, id) {
  ...
  const params = {
    Bucket: process.env.AWS_S3_BUCKET_NAME,
    // Our key is the same mix of ownerID and fragment id used when writing
    Key: `${ownerId}/${id}`,
  };
  ...
  try {
    // 
    // Use Promise.all to delete from both S3 and the MemoryDB in parallel.
    // We await the result so we can catch any errors that occur in either operation.
